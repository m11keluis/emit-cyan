...
def ortho_xr(ds, GLT_NODATA_VALUE=0, fill_value=-9999):
    ...
    coords = {'latitude': (['latitude'], lat), 'longitude': (['longitude'], lon),
              **ds.coords}  # unpack to add appropriate coordinates
...
def write_envi(xr_ds, output_dir, overwrite=False, extension='.img', interleave='BIL', glt_file=False):
    ...
    mapinfo = '{Geographic Lat/Lon, 1, 1, ' + str(gt[0]) + ', ' + str(gt[3]) + ', ' + str(gt[1]) + ', ' + str(
        gt[5] * -1) + ', WGS-84, units=Degrees}'
...
def envi_header(inputpath):
    ...
    """
    if os.path.splitext(inputpath)[-1] == '.img' or os.path.splitext(inputpath)[-1] == '.dat' or \
            os.path.splitext(inputpath)[-1] == '.raw':
...
def raw_spatial_crop(ds, shape):
    ...
    data_vars = {'glt_x': (['latitude', 'longitude'], ds.glt_x.data),
                 'glt_y': (['latitude', 'longitude'], ds.glt_y.data)}
    coords = {'latitude': (['latitude'], lat), 'longitude': (['longitude'], lon),
              'ortho_y': (['latitude'], ds.ortho_y.data), 'ortho_x': (['longitude'], ds.ortho_x.data)}
    attrs = ds.attrs
    ...
    clipped_gt = np.array(
        [float(i) for i in clipped['spatial_ref'].GeoTransform.split(' ')])  # THIS GEOTRANSFORM IS OFF BY HALF A PIXEL
    ...
    crosstrack_mask = (ds.crosstrack >= np.nanmin(clipped.glt_x.data) - 1) & (
                ds.crosstrack <= np.nanmax(clipped.glt_x.data))
    downtrack_mask = (ds.downtrack >= np.nanmin(clipped.glt_y.data) - 1) & (
                ds.downtrack <= np.nanmax(clipped.glt_y.data))
    ...
    clipped_ds = clipped_ds.assign_coords({'glt_x': (['ortho_y', 'ortho_x'], np.nan_to_num(glt_x_data)),
                                           'glt_y': (['ortho_y', 'ortho_x'], np.nan_to_num(glt_y_data))})
    clipped_ds = clipped_ds.assign_coords(
        {'downtrack': (['downtrack'], np.arange(0, clipped_ds[list(ds.data_vars.keys())[0]].shape[0])),
         'crosstrack': (['crosstrack'], np.arange(0, clipped_ds[list(ds.data_vars.keys())[0]].shape[1]))})
